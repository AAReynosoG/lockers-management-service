...
export default class LogsController {
  async storeAccessLogs({ request, response }: HttpContext) {
    ...
    try {
      ...
      const userIds = [...new Set(logs.map((log) => log.user_id))].filter(
      ...
      let userRolesQuery = LockerUserRole.query().whereIn(
        'locker_id',
        lockersData.map((l) => l.id)
      )

      if (userIds.length > 0) {
        userRolesQuery = userRolesQuery.whereIn('user_id', userIds)
      }

      const userRolesData = await userRolesQuery.preload('user')

      const lockerMap = new Map(lockersData.map((l) => [l.serialNumber, l]))
      const userRoleMap = new Map(userRolesData.map((ur) => [`${ur.lockerId}-${ur.userId}`, ur]))
          ...
          return {
            ...
            locker: locker
              ? {
                  ...
                  manipulated_compartment: compartment_number ?? null,
      ...
      for (const [serialNumber, serializLogs] of logsBySerial) {
        UnifiedBackgroundProcessor.addLogs(serializLogs, 'lockers_logs', true, true, serialNumber)
      }
  ...
  private async getLogsBase(
    ...
    const compartment = request.input('compartment')
      ? Number(request.input('compartment'))
      : undefined
  ...
  async getAuditLogs(ctx: HttpContext) {
    ...
    const result = await this.getLogsBase(ctx, 'audit_logs', ['admin', 'super_admin'])
  ...
  async getLockerActivities(ctx: HttpContext) {
    ...
    const organizationId = request.input('organizationId')
      ? Number(request.input('organizationId'))
      : null
    ...
    if (!['success', 'failure'].includes(status)) {
      return sendErrorResponse(
        response,
        400,
        'Invalid status parameter. Must be "success" or "failure".'
      )
    }
    ...
    if (organizationId !== null && (isNaN(organizationId) || organizationId <= 0)) {
      return sendErrorResponse(
        response,
        400,
        'Invalid organizationId parameter. Must be a positive number.'
      )
    }
